...
IRAM_ATTR bool onLcdVsyncCallback(void *user_data)
{
    ...
    // Guard: only notify if the task handle is still valid (not deleted)
    if (task_handle != nullptr && lvgl_task_handle != nullptr) {
...
static bool lvgl_task_paused = false;
...
static void lvgl_port_task(void *arg)
{
    ...
    while (1) {
        if (!lvgl_task_paused && lvgl_port_lock(-1)) {
...
{
    lvgl_task_paused = true;
}

void lvgl_port_resume(void)
{
    lvgl_task_paused = false;
}
